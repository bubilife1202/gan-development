...
- **리스크 레지스터** (Product + Design GAN에서 갱신된 최신 버전)
  - 병합 규칙: 동일 RISK-ID에 대해 양쪽의 상태가 다르면, 더 보수적인 상태(미완화)를 채택한다.
  - 각 GAN에서 새로 추가된 리스크는 합집합으로 포함한다.
  - 미완화 리스크는 반드시 구현에서 고려한다.
...
### 입력 충돌 해소 규칙
Product GAN 산출물과 Design GAN 산출물 간 충돌 발견 시:
1. **기능/행동 관련 충돌:** Product GAN 결정이 우선 (기능 동작은 Product GAN의 영역)
2. **시각/스타일 관련 충돌:** Design GAN 결정이 우선
3. **판단 불가한 충돌:** 사용자에게 보고하고 판단 요청
...
| 차원 | 공격 대상 |
|------|----------|
| 보안 | 인증 우회, 인가 우회 (IDOR/BOLA), 인젝션, XSS, CSRF, SSRF, RLS, 권한 상승, 시크릿 노출, 역직렬화/프로토타입 오염, 대량 할당, 의존성 취약점 |
| 정확성 | 레이스 컨디션, 엣지케이스, 데이터 무결성, 트랜잭션 |
| 성능 | N+1 쿼리, 페이지네이션, 번들 사이즈, 렌더 사이클 |
| 타입 | 안전하지 않은 캐스팅, 검증 누락, 스키마 불일치 |
| 에러 | 미처리 실패, 조용한 에러, 사용자 피드백 부재, 에러 메시지를 통한 내부 정보 노출 |
